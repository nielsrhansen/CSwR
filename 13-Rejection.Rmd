...
outer for-loop over the number of observations we want, and an inner while-loop
...
The [profiling result](figures/vMsim_profile.html) shows that the majority
of the time is spent on simulating uniformly distributed random variables. It is, perhaps, 
...
The function performs better if called once to return a vector than 
...
random number of accepted proposals instead of `n` observations. Since we do not 
...
a fixed number of observations. We will need to solve this problem for 
several other distributions, so we implement a general solution 
that can convert any sampler generating a random number of observations into 
a sampler that returns a fixed number of observations. 
...
vectors of accepted observations. It is usually considered bad practice to dynamically 
grow objects (vectors or list), as this will result in unnecessary memory 
allocation, copying and deallocation. If possible, it is better to initialize a vector
...
In addition to the vectorized implementation, we give a C++ implementation via Rcpp 
below, where the random variables are 
...
is a factor 40 or so slower than the two others. Rejection sampling is a good example 
...
Rejection sampling can be computationally expensive if many observations are rejected. 
...
\@ref(exr:laplace-sim) and \@ref(exr:laplace-sim-2)). Test the implementation
by computing the empirical mean and variance of the simulated variables and
compare with those of the Gaussian distribution. Compare also directly with the
Gaussian distribution using histograms and QQ-plots. 
