...
$H: \Theta \to \mathbb{R}$ for $\Theta \subseteq \mathbb{R}^p$ an open set and $H$
...
a subtle tradeoff between numerical accuracy and accuracy of the finite
...
$H$ is \emph{strongly convex}, there is a constant $C$ such that 
...
that $\theta_n$ is close to $\theta_{\infty}$---if $\epsilon$ is 
small relative to $C$. Another approach is based on a lower 
envelope. If there is a continuous function $\tilde{H}$ satisfying 
...
gridExtra::grid.arrange(p1 + cc, p2 + cc, ncol = 2)
```
