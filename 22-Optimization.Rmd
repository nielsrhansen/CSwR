...
Geometrically, a gradient descent step replaces $\theta_{n-1}$ with a point
displaced opposite to the local slope, by an amount proportional to its
magnitude. Figure\ \@ref(fig:gd-step-1d) shows one such step on a
one-dimensional convex $H$, written with a generic step length $t > 0$ in
place of the fixed $1/(L + 1)$ above.
...
The step length is important for the convergence of the gradient descent
algorithm. If the step length is too small, the algorithm will converge slowly.
If the step length is too large, the algorithm may diverge. Figure
\@ref(fig:gd-step-sizes) illustrates three situations on a simple quadratic
in two dimensions, whose Hessian has eigenvalues $1$ and $L = 4$. A step far
below $1/L$ crawls toward the optimum, a step at $1/L$ converges in a handful
of iterations, and a step above the stability boundary $2/L$ diverges.
...
#| fig-height: 3
#| fig-cap: >-
#|   Gradient descent on the quadratic problem $H(\theta) = (\theta_1^2 + 4\theta_2^2)/2$ (so $L = 4$)
#|   for three step sizes. A step much smaller than $1/L$
#|   descends slowly whereas a step at $1/L$ converges efficiently. And a step above
...
Figure\ \@ref(fig:armijo) shows the condition graphically. The line
$h(0) + c \gamma h'(0)$ has a shallower slope than the tangent at $0$, and
sufficient descent holds for those step lengths where $h$ falls below it.
...
#| fig-align: "center"
#| fig-cap: >-
#|   The sufficient descent condition, also known as the Armijo condition.
#|   Solid: the restriction $h(\gamma) = H(\theta + \gamma
...
#|   interval marks the values of $\gamma$ for which the sufficient descent
#|   condition
...
Both algorithms can be read as minimizing a local quadratic model of $H$. For
gradient descent with step length $t$, that model is the second-order Taylor
expansion around $\theta_n$ with the Hessian replaced by $t^{-1} I$, and its
minimizer is exactly $\theta_n - t \nabla H(\theta_n)$ (see Figure
\@ref(fig:gd-taylor-surrogate)). 
...
Newton's method keeps the true Hessian, so its
model matches the local curvature of $H$ instead of assuming that the curvature
is the same in every direction. Figure \@ref(fig:gd-vs-newton-surrogate)
contrasts the two on an ill-conditioned least squares problem.
...
#| fig-height: 4.5
#| fig-cap: >-
#|   Surrogate models and iterates for the OLS objective on an ill-conditioned
#|   design whose two columns have correlation $0.8$. Solid grey contours are level
#|   sets of $H(\beta) = \tfrac{1}{2}\lVert y - X\beta \rVert^2$ whilst dashed blue
