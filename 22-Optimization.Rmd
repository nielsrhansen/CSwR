...
the likelihood is unbounded, yet there will be a local maximizer that is a 
good estimate. There are other examples where even a saddle point of $H$ 
gives a good estimate. Though we typically phrase our algorithms 
...
to the greatest numerical precision, we should find and explore as many 
...
the algorithms that will be introduced in subsequent chapters as well 
...
stationary point in $\mathrm{lev}(\theta_0)$, then this is, in fact, 
$\theta_{\infty}$, it is a minimizer, and 
...
cc1 <- coord_cartesian(xlim = c(0, 90), ylim = c(1e-4, 100))
cc2 <- coord_cartesian(xlim = c(0, 90), ylim = c(1e-10, 100))
