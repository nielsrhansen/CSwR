...
In terms of the probability parameters $p_C$ and $p_I$ this is a curved 
exponential family. 

Collecting moths and determining their color will only identify their phenotype, 
not their genotype. Thus we only observe $(n_C, n_T, n_I)$, where
...
The conditional distribution of $Y_{A_j} = (Y_k)_{k \in A_j}$, conditionally on 
$X$, can be found too. It will be used for the EM algorithm 
...
The `optim` functions uses an algorithm called [Nelder-Mead](https://en.wikipedia.org/wiki/Nelder–Mead_method) 
...
## Variance 
sigmasq <- p * sigma1^2 + (1 - p) * sigma2^2 + p * (1-p)*(mu1^2 + mu2^2 - 2 * mu1 * mu2)
