...
### Full exponential families {#exp-fam}
...
An exponential family is defined in terms of two ingredients: 
  ...
  * maps $t_j : \mathcal{Z}_j \to \mathbb{R}^p$ for $j = 1, \ldots, m$,
  * and non-trivial $\sigma$-finite measures $\mu_j$ on $\mathcal{Z}_j$ for $j = 1, \ldots, m$.


The maps $t_j$ are called *sufficient statistics*, and in terms 
of these and the *base measures* $\mu_j$ we define 
$$\varphi_j(\theta) = \int e^{\theta^T t_j(u)} \mu_j(\mathrm{d}u).$$
...
that $\Theta_j$ is convex and that $\varphi_j$ is a log-concave function. 
Defining 
...
\begin{equation}
f(\mathbf{z} \mid \theta) = \prod_{j=1}^m \frac{1}{\varphi_j(\theta)} e^{\theta^T t_j(z_j)} = e^{\theta^T \sum_{i=1}^m t_j(z_j) - \sum_{i=1}^m \log \varphi_j(\theta)}, \quad \mathbf{z} \in \mathcal{Z},
(\#eq:exp-dens)
\end{equation}

w.r.t. $\otimes_{j=1}^m \mu_j$. The 
...
The von Mises distributions on $\mathcal{Z} = (-\pi, \pi]$ form an exponential family
with $m = 1$. The sufficient statistic $t_1 : (-\pi, \pi] \mapsto \mathbb{R}^2$ 
is 
$$t_1(z) = \left(\begin{array}{c} \cos(z) \\ \sin(z) \end{array}\right),$$
and 
$$\varphi(\theta) = \int_{-\pi}^{\pi} e^{\theta_1 \cos(z) + \theta_2 \sin(z)} \mathrm{d}z < \infty$$
...
family as defined above with $m = 1$ and $\mathcal{Z} = \mathbb{R}$. 
The density of the $\mathcal{N}(\mu, \sigma^2)$ distribution is 
$$\frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{(z - \mu)^2}{2\sigma^2}\right) = 
  \frac{1}{\sqrt{2\pi}} \exp\left(\frac{\mu}{\sigma^2} z - \frac{1}{\sigma^2} \frac{z^2}{2} - \frac{\mu}{2\sigma^2} - \log \sigma^2\right).$$
Letting the base measure $\mu_1$ be Lebesgue measure scaled by $1/\sqrt{2\pi}$,
and $t_1 : \mathbb{R} \mapsto \mathbb{R}^2$ be 
$$t_1(z) = \left(\begin{array}{c} z \\ -\frac{z^2}{2} \end{array}\right)$$
we identify this family of distributions as an exponential family with canonical
parameter 
$$\theta = \left(\begin{array}{c} \frac{\mu}{\sigma^2} \\ \frac{1}{\sigma^2} \end{array}\right).$$

We can express the mean and variance in terms of $\theta$ as 
$$\sigma^2 = \frac{1}{\theta_2} \quad \text{and} \quad \mu = \frac{\theta_1}{\theta_2},$$
and we find that 
$$\log \varphi_1(\theta) = \frac{\mu}{2\sigma^2} + \log \sigma^2 = \frac{1}{2}\theta_1 - \log \theta_2.$$
...
$$\varphi_1(\theta) = \frac{1}{\sqrt{2\pi}} \int e^{\theta_1 z - \theta_2 \frac{z^2}{2}} \mathrm{d}z
\geq \frac{1}{\sqrt{2\pi}} \int e^{\theta_1 z} \mathrm{d} z = \infty,$$
...
allowed to depend on $z_1, \ldots, z_{j-1}$, $f(\mathbf{z} \mid \theta)$ as 
defined in \@ref(eq:exp-dens)
is still a joint density. The only difference is that the factor 
$$f_j(z_j \mid z_1, \ldots, z_{j-1}, \theta) = e^{\theta^T t_j(z_j) - \log \varphi_j(\theta)}$$
is now the *conditional* density of $z_j$ given $z_1, \ldots, z_{j-1}$.
In the notation we let the dependence of $t_j$ on $z_1, \ldots, z_{j-1}$ 
...
upon the data through $z_1, \ldots, z_{j-1}$.
...
assumptions 
  ...
  1. the variables that we model can be ordered such that $z_j$ only 
  depends on $z_1, \ldots, z_{j-1}$ for $j = 1, \ldots, m$,
...
the situation where $z_1, \ldots, z_m$ are i.i.d. by taking $t_j$ to be 
independent of $j$. In that case, $\kappa(\theta) = m \kappa_1(\theta)$,
the score equation can be rewritten as 
$$\frac{1}{m} \sum_{j=1}^m t_1(z_j) = \kappa_1(\theta),$$
...
have i.i.d. replications $\mathbf{z}_1, \ldots, \mathbf{z}_n$ of
the entire $m$-dimensional vector $\mathbf{z}$, and we would get 
the score equation 
$$\frac{1}{n} \sum_{i=1}^n t(\mathbf{z}_i) = \kappa(\theta),$$
...
a finite set $\mathcal{Z} = \{1, \ldots, K\}$. The model is parametrized
by the simplex 
...
In the special case of $K = 2$ the two elements $\mathcal{Z}$ are often given 
...
thus 
$$p_1 = \frac{e^{\theta}}{1 + e^{\theta}} = e^{\theta} p_0 = e^{\theta} (1 - p_0).$$
...
$$p_z = \frac{e^{k\theta}}{e^\theta + e^{-\theta}}$$
...
pollution, where trees were darkened by soot. 
...
models are based on exponential families. 
...
pois_model_null <- glm(sale ~ log(normalSale), 
                       data = vegetables, 
                       family = poisson)   ## Fitting a Poisson regression model
```
