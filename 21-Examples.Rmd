...
\begin{equation}
f(\mathbf{y} \mid \theta) = \prod_{j=1}^m \frac{1}{\varphi_j(\theta)} e^{\theta^T t_j(y_j)} = e^{\theta^T \sum_{i=1}^m t_j(y_j) - \sum_{i=1}^m \log \varphi_j(\theta)}, \quad \mathbf{y} \in \mathcal{Y},
(\#eq:exp-dens)
\end{equation}
...
and 
$$\varphi(\theta) = \int_{-\pi}^{\pi} e^{\theta_1 \cos(y) + \theta_2 \sin(y)} \mathrm{d}z < \infty$$
...
$$\frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{(y - \mu)^2}{2\sigma^2}\right) = 
  \frac{1}{\sqrt{2\pi}} \exp\left(\frac{\mu}{\sigma^2} y - \frac{1}{\sigma^2} \frac{y^2}{2} - \frac{\mu^2}{2\sigma^2} - 
...
and we find that 
$$\log \varphi_1(\theta) = \frac{\mu^2}{2\sigma^2} + \frac{1}{2} \log \sigma^2 = 
...
$$Y_{A_j} \mid X = x \sim \textrm{Mult}\left( \frac{p_{A_j}}{M(p)_j}, y_j \right).$$
...
$$E (Y_k \mid X = x) = \frac{y_j p_k}{M(p)_j}$$
for $k \in A_j$. 
...
model as $K$ mixture components and call $f_k( \cdot \mid \theta_k)$
...
the mixture weight $p_1 \in (0, 1)$, the two means $\mu_1, \mu_2 \in \mathbb{R}$, 
...
Gaussian mixture model with mixture distributions $\mathcal{N}(0, 1)$
...
\begin{equation}
Y = 1(Z = 1) Y_1 + 1(Z = 2) Y_2.
(\#eq:mixturerep)
\end{equation}

You can argue from this that the conditional distribution of $Y $given $Z = k$
indeed is $f_k$, or you can just verify directly that $Y$ has density 
$$f(y) = p \frac{1}{\sqrt{2 \pi \sigma_1^2}} e^{-(y - \mu_1)^2 / (2 \sigma_1^2)} + 
(1 - p)\frac{1}{\sqrt{2 \pi \sigma_2^2}}e^{-(y - \mu_2)^2 / (2 \sigma_2^2)}.$$
From \@ref(eq:mixturerep) it follows directly that
$$E(Y) = P(Z = 1) E(Y_1) + P(Z = 2) E(Y_2) = p_1 \mu_1 + (1 - p_1) \mu_2$$
and 

\begin{align}
V(Y) & = V(1(Z = 1) Y_1) + V(1(Z = 2) Y_2) \\
& = P(Z = 1) E(Y_1^2) - P(Z = 1)^2 E(Y_1)^2 + P(Z = 2) E(Y_2^2) - P(Z = 2)^2 E(Y_2)^2 \\
& = p(\sigma_1^2 + \mu_1^2) - p^2 \mu_1^2 +  (1-p)(\sigma_2^2 + \mu_2^2) - (1-p)^2 \mu_2^2 \\
& = p\sigma_1^2 + (1 - p) \sigma_2^2 + p(1-p)(\mu_1^2 + \mu_2^2).
\end{align}
...
elementary properties of mean and variance together with the independence
of $Y_1$, $Y_2$ and $Z$.  
...
an example. Suppose that we have a population consisting of married couples entirely.
...
implementations of ways to simulate from the mixture model. 
...
## Variance 
sigmasq <- p * sigma1^2 + (1 - p) * sigma2^2 + p * (1-p)*(mu1^2 + mu2^2)
...
structure and one have to deal with the models much more on a case-by-case
manner.
