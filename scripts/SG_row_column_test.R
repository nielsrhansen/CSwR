...
m <- 50
set.seed(10)
grad1 <- ls_news$grad(par, sample.int(ls_news$N, m))
set.seed(10)
grad2 <- ls_news_t$grad(par, sample.int(ls_news_t$N, m))
set.seed(10)
grad3 <- grad_cpp(par, sample.int(ls_news$N, m), ls_news$X, ls_news$y)
set.seed(10)
grad4 <- grad_cpp_t(par, sample.int(ls_news$N, m), ls_news_t$X, ls_news_t$y)
...
bench::mark(
  ord = ls_news$grad(par, sample.int(ls_news$N, m)),
  trans = ls_news_t$grad(par, sample.int(ls_news$N, m)),
  ord_rcpp = grad_cpp(par, sample.int(ls_news$N, m), ls_news$X, ls_news$y),
  trans_rcpp = grad_cpp_t(
    par,
    sample.int(ls_news$N, m),
    ls_news_t$X,
    ls_news_t$y
  ),
  check = FALSE
