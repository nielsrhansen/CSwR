...
$g \propto 1$. Since $x \mapsto \exp(\kappa \cos(x))$ attains it maximum for 
$x = 0$ we find that 
$$e^{\kappa(\cos(x) - 1)} = \alpha' e^{\kappa \cos(x)} \leq 1,$$
where $\alpha' = \exp(-\kappa)$. The rejection test of the 
...
```{r vMsim, fig.cap="(ref:vMsim)", dependson=c("vonMisesDens", "vM-rejection"), fig.pos="h", fig.show='hold', out.width="50%"}
...
(ref:vMsim) Histograms of `r length(x)` simulated data points from von Mises 
distributions with parameters $\kappa = 0.5$ (left) and $\kappa = 2$ (right). 
The true densities (blue) are added to the plots.

Figure \@ref(fig:vMsim) confirms that the implementation simulates
from the von Mises distribution. 

```{r vMsim-runtime, dependson="vM-rejection-fast"}
system.time(vMsim(100000, kappa = 5))
```

Though the implementation can easily simulate 100000 variables in a couple of 
...
```{r profvis-vMsim-image, out.width="100%", echo=FALSE}
knitr::include_graphics("figures/vMsim-profile.png")
```

The [profiling result](vMsim-profile.html) shows that almost all the time 
...
if called repeatedly as above to return just single numbers. We can 
rewrite the rejection sampler to make better use of `runif`, but it does make
the function a bit more complicated because we don't know upfront 
how many uniform variables we need. For later usage we add the 
...
The implementation above generates new vectors of uniform random variables 
whenever it "runs out". The first time it needs to do so it estimates a 
factor of how many variables is needed per accept, and then it generates 
the estimated number of variables needed. This may be repeated a couple of 
times.

```{r vMsim-fast-runtime, dependson="vM-rejection-fast"}
system.time(vMsim(100000, kappa = 5))
```

As we see from the time estimate above, using a vectorized call of `runif` 
reduces the run time by more than a factor 10. 
...
```{r vMsim2, fig.cap="(ref:vMsim2)", dependson=c("vMsim", "vonMisesDens", "vM-rejection-fast"), fig.pos="h", fig.show='hold', out.width="50%"}
x <- vMsim(100000, 0.5)
hist(x, breaks = seq(-pi, pi, length.out = 20), prob = TRUE)
curve(f(x, 0.5), -pi, pi, col = "blue", lwd = 2, add = TRUE)
x <- vMsim(100000, 2)
...
(ref:vMsim2) Histograms of `r length(x)` simulated data points from von Mises 
distributions with parameters $\kappa = 0.5$ (left) and $\kappa = 2$ (right),
simulated using the fast implementation.
...
```{r gammaBench, echo=2, dependson="gammasim"}
y <- gammasim(1, 8)
system.time(y <- gammasim(100000, 8))
...
system.time(y <- gammasim(100000, 1))
...
This section develops first of all a method for construction of envelopes
...
but it is not uncommon in practice, and it can be extended to work 
...
more than 80% of the proposals are rejected, and simulating $n = 10000$ 
von Mises distributed variables thus requires the simulation of around $50000$
...
factor 4, but there is no actual speedup. 
