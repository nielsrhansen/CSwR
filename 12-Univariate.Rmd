...
this generator is given iteratively by
...
use. They are mostly availabel for historical reasons. 
...
$$z = z_{31} 2^{31} + z_{30} 2^{30} + \ldots + z_2 2^3 + z_1 2^{1} + z_0.$$
...
The number 362437 is Masaglia's choice for generating what he calls a
...
below we achive this by simply exporting a direct interface to the xorwow generator.
...
RCPP_MODULE(xorwow) {
  Rcpp::function("xor_set.seed", &user_unif_init, "Seeds Masaglia's xorwow");
  Rcpp::function("xor_runif", &xor_runif, "A uniform from Masaglia's xorwow");
}
```
...
of xorwow above is experimental and has not been throughly tested, we will 
