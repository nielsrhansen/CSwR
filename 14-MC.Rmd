...
of $\hat{\mu}_{\textrm{MC}}$ to assess the accuracy by computing a confidence 
...
plot(cumsum(evaluations) / 1:B, pch = 20, xlab = "n")
abline(h = 1, col = "red")
```
...
plot(cumsum(evaluations) / 1:B, pch = 20, ylim = c(0, 2), xlab = "n")
me <- cumsum(evaluations) / 1:B
ve <- var(evaluations)
abline(h = 1, col = "red")
lines(1:B, me + 2*sqrt(ve/(1:B)))
...
replicate(4,
  {x <- matrix(rnorm(B * p), B, p)
  evaluations <- apply(x, 1, h, alpha = 0.4)
  plot(seq(1, B, 10), (cumsum(evaluations) / 1:B)[seq(1, B, 10)], 
       pch = 20, ylim = c(0, 2), xlab = "n")
  me <- cumsum(evaluations) / 1:B
  ve <- var(evaluations)
  abline(h = 1, col = "red")
  lines((1:B)[seq(1, B, 10)], (me + 2*sqrt(ve/(1:B)))[seq(1, B, 10)])
  lines((1:B)[seq(1, B, 10)], (me - 2*sqrt(ve/(1:B)))[seq(1, B, 10)])
  })
```
...
disconAB <- function(A) {
  ...
  Apow[1, 10] == 0  ## TRUE if 1 and 10 are not connected
}
```

Estimating probability of nodes 1 and 10 being disconnected using Monte Carlo integration. 
