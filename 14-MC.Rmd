...
If $h_0(x) = h(x) f(x)$ for a probability density $f$, we can get the
idea of choosing the $x_i$-s randomly and simply take $w_i = 1/N$. With
$X_1, \ldots, X_N$ i.i.d. with density $f$, the *Monte Carlo* average satisfies
that
...
integral. The idea may be applied in a statistical context, but it also has
...
question of Monte Carlo integration is therefore to quantify the precision of
the average.

In this chapter, we we will first deal with the quantification of the
precision---primarily via the asymptotic variance in the central limit
theorem. This will provide both a quantification of precision for any specific
Monte Carlo approximation as well as a way to compare different Monte Carlo
...
This might be difficult or impossible, or it might just lead to Monte Carlo
averages of very low precision. To remedy these challenges the subsequent
sections treat importance sampling and variations thereof, which are techniques
for simulating from a different distribution and use a *weighted* average to
obtain the approximation of the integral.
...
of $\hat{\mu}_{\textrm{MC}}$ to assess the precision by computing a
confidence interval, say. Or we can provide finite sample upper bounds,
...
inequality can be turned into a confidence interval, if needed, or it can be
used directly to answer a question such as: if I want the approximation to have
an error smaller than $\varepsilon = 10^{-3}$, how large does $N$ need to be to
...
They are, however, typically problem specific and harder to derive, they
involve constants that are difficult to compute or bound, and they tend to be
pessimistic in real applications.
...
specific constant (the variance) that enter into the bound it provides.
...
via the CLT by computing the mean of a Gamma distribution via Monte Carlo
integration.
...
Monte Carlo average. The CLT states in this particular case that
...
A common usage of this inequality is to actually prove the *Law of Large
Numbers*: for any $\varepsilon > 0$
...
Chebychev's inequality does, additionally, give a a quantitative statement
...
Solving for $1/\varepsilon^2 = 0.05$, say, gives
$\epsilon = \sqrt{1 / 0.05} = 4.47$, so if we knew $\sigma_{\textrm{MC}}$ the
confidence interval
...
The choice of the proposal distribution $g$ is crucial for the performance of
importance sampling. So far, we have offered some
...
#| label: MC-CLT3
#| fig-cap: "Four sample paths of the cumulative average for $\\alpha = 0.25$."
#| echo: false
#| results: "hide"
#| out-width: "100%"
