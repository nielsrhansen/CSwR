...
context, but it also has applications outside of statistics and can be 
...
In this chapter, we we will first deal with the quantification of the precision---primarily 
via the asymptotic variance in the central limit theorem. This will provide 
both a quantification of precision for any 
specific Monte Carlo approximation as well as a way to compare different Monte Carlo integration techniques. 
...
The choice of the proposal distribution $g$ is crucial for the performance of importance sampling.
So far, we have offered some 
...
the $Z$-s form a Markov chain. The equation \@ref(eq:HMM-observation) is 
...
is the density of $Z_{0:k}$ w.r.t. Lebesgue measure in $\mathbb{R}^k$. 
Likewise, 
