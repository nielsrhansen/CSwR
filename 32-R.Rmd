...
lead to faster computations than an explicit loop
...
vectorized functions (functions that operate on vectors)---but that notion is
misleading. It is true that vectorized functions are often much faster than
R-based for loops, but the reason for that is not that they operate on vectors
or eschew the use of loops. On the contrary, many vectorized functions are
implemented with loops in C or Fortran. Moreover, many R functions that are
vectorized in fact are implemented in R-based loops, which means that they are
not faster than hand-written loops. In fact, the opposite if often the case, as
