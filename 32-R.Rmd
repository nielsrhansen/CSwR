...
functions. Sometimes, trying to write R code that is fast may also lead to code
that is difficult to read and understand. In these cases, the solution is often
to write the critical parts of the code in a compiled language. R has built-in
support for calling multiple compiled languages, such as C, C++, and Fortran.
Doing this with the built-in functions `.C()`, `.Call()`, and `.Fortran()` is,
however, seldom ergonomic. Thankfully, it is much easier to write compiled code
in C++ and call it from R using the Rcpp package, which is what we will briefly
introduce here.
