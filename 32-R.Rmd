...
bench::mark(
  my_mean = my_mean(rnorm(1e6)),
  base_mean = mean(rnorm(1e6)),
  check = FALSE
) |>
  plot()
```
...
[Vectorization])---but that notion is misleading. It is true that vectorized
...
The *actual* reason for why vectorization often help therefore has nothing at
all to do with vectorization itself, but is instead that they are implemented in
a compiled language. And it is this property that we should try to profit from.
