...
are not covered in any detail in this appendix. The reader is, in particular,
...
`double_vector`, from 0.1 to 1.0 with increments of size 0.1. The type of the
...
is the more permissive number $\sqrt{2^{-53}} \approx 1.5 \times 10^{-8}$. This
...
arguments, see Section \@ref(funfac), this is technically a little imprecise).
...
how much time was spent on each line of code within one or more functions during
evaluation.

Running the following code will profile the implementation of the random number
stream and store the result in as an interactive HTML widget. The widget can be
...
entire source code of that function as part of the expression. If
`runif_stream()` is implemented in a file, which is read into R via the
...
a large number of repeated calls of each function. This is to make the execution 
...
objective is to be able to store values of one or more variable from inside a
...
```{r plot-mean-trace, dependson="mean-loop-tracer-2"}
