...
advantage of this property in computations by parallelizing our operations,
splitting into multiple parts and running each separately on different cores. It
is easy to see how this can speed up computations.

The problem, however, is that parallelization is not inherently safe. Not every
algorithm or computation is amenable to parallelization simply because order of
executtion sometimes matters. Therefore, your computer will not automatically
know how to parallelize your code, and every for loop your write will therefore,
by default, be executed sequentially (on a single core)---unless you explicitly
tell R to parallelize it, which is what we will learn how to do next.

There is a plethora of packages for parallel computing in R and we will not
cover all of them here. Instead, we will focus on two packages:
[`foreach`](https://cran.r-project.org/package=foreach) and
[`future`](https://cran.r-project.org/package=future), which are both very
popular and easy to use, but cover slightly different approaches to
parallelization.

The `foreach` package provides a simple, close-to drop-in, replacement for the
`for` loop, which allows you to parallelize the computations in the loop with
minimal changes to your code.

The idea if that you replace a for loop with the following construct:
...
This is designed to look and feel like a regular for loop---but it is not. The
`foreach()` function is a function that returns a special object that is then
evaluated with the `%dopar%` operator, which in turn returns a list. This is
fundamentally different from a regular for loop, which does not return anything
at all but instead modifies objects in place. This is necessary because the
computations in the loop are executed on different cores, and the results need
to be collected and returned in a way that is compatible with this parallel
execution.

Note that the chunk above returned a warning about having no parallel backend
registered. This is because the `foreach` package needs a parallel backend to
actually run the computations in parallel. There are several parallel backends
available for `foreach`, such as `doParallel`, `doMC`, and `doFuture`, which you
can install and register to enable parallel execution.
