...
which we usually think of as a collection of functions $K_h : \mathbb{R}
\to [0,\infty)$ parametrized by $h > 0$. Though $x$ and $h$ in this way play
...
environments. We will refer to any such constructor function as a
...
[interactive HTML](figures/rng_stream_profile.html) widget shows the data in
the form of a flame graph,\index{flame graph} and it includes an alternative
summarized view of the profiling data. The profiling data itself is collected
by `profvis()`\index{profvis()} via the the `Rprof()` function from the utils
...
::: {.exercise #plotHist}
Plot a histogram of `log(F12)` using the default value of the argument
...
vector `c(1, 1, 1, 2, 3, 3, 5, 10, 11)`, whose unique values are `c(1, 2, 3, 5,
10, 11)`. Every second unique entry is `c(1, 3, 10)`, and then the largest
entry `11` is concatenated.
