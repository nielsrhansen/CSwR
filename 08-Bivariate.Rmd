...
ggplot(Nuuk_year, aes(x = Year, y = Temperature)) + 
...
ggplot(Nuuk, aes(x = Year, y = Temperature)) + 
  geom_point() + geom_smooth(se = FALSE) + 
  geom_smooth(method = "lm", formula = y ~ poly(x, 10), color = "red", se = FALSE) + 
  geom_smooth(method = "gam", formula = y ~ s(x, bs = "cr"), color = "purple", se = FALSE) +
  facet_wrap(~ Month)
...
Abstract view on running means as linear smoother. Dense linear algebra 
implementation.

### Dense linear algebra


Many linear smoothers can be computed much faster than via a matrix multiplication 
of the smoother matrix and the observation vector, which has time complexity $O(n^2)$. 
...
y <- rnorm(7)
rbind(runMean(y, k = 3),
c(stats::filter(y, rep(1/3, 3))))
rbind(runMean(y, k = 5),
c(stats::filter(y, rep(1/5, 5))))
```

A benchmark comparison between `runMean` and `filter` gives 
the following table. 

```{r runMeanBench, dependson="runMean", echo=FALSE}
y <- rnorm(4196)
tmp <- microbenchmark(
...
benchTab(summary(tmp)[, c(1, 4, 5)], digits = 1)
```

Thus `filter` is somewhat faster than `runMean`.  
...
```{r fastBand_R, dependson="Rcpp_depends"}
...
```{r model_matrix_fig, echo=FALSE, dependson=c("NuukData", "Nuuk_model_matrix"), fig.cap="The model matrix columns as functions"}
XX <- X
...
```{r smoothMat_fig, echo=FALSE, dependson="SmoothMat", fig.cap="Smoothers"}
...
```{r Nuuk_smooth, echo=FALSE, dependson=c("Rcpp_depends", "NuukData", "Nuuk_runmeans")}
...
```{r Nuuk_filter, echo=FALSE, dependson=c("Rcpp_depends", "NuukData", "Nuuk_runmeans")}
