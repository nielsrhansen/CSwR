...
With $\mathbf{f}$ denoting the vector of smoothed values by a nearest 
neighbor smoother we can observe that it is always possible to write 
$\mathbf{f} = \mathbf{S}y$ for a matrix $\mathbf{S}$. For the symmetric 
...
A smoother of the form $\mathbf{f} = \mathbf{S}y$ for a *smoother matrix* $\mathbf{S}$, 
...
with $\boldsymbol{\Phi}_{ij} = \phi_j(x_i)$, and
...
$$\hat{\mathbf{f}} = \boldsymbol{\Phi} ((\boldsymbol{\Phi}^T \boldsymbol{\Phi} + \lambda \mathbf{\Omega})^{-1}\boldsymbol{\Phi}^T \mathbf{Y}.$$
...
One hurdle is the computation of the matrix $\Omega$. 
...
```{r}
fda::bsplinepen(fda::create.bspline.basis(c(0, 1), breaks = seq(0, 1, 0.1)))
```
