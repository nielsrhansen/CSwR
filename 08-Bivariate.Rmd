...
neighborhood, $B_i$, say, around $x_i$. This would lead to the estimator 
$$\hat{f}_i = \frac{\sum_{j} y_j 1_{B_i}(x_j)}{\sum_{j} 1_{B_i}(x_j)},$$
...
In a metric space a natural 
choice of $B_i$ is the ball, $B(x_i, h)$, around $x_i$ with radius $h$. 
In $\mathbb{R}$ with the usual metric (or even $\mathbb{R}^p$ equipped with any 
translation invariant metric) we have that  
$$1_{B(x_i, h)}(x) = 1_{B(0, 1)}\left(\frac{x - x_i}{h}\right),$$
thus since $B(0, 1) = [-1, 1]$ in $\mathbb{R}$
$$\hat{f}_i = \frac{\sum_{j} y_j 1_{[-1, 1]}\left(\frac{x_j - x_i}{h}\right)}{\sum_{l} 1_{[-1, 1]}\left(\frac{x_l - x_i}{h}\right)}.$$
This nonparametric estimator of the conditional expectation $E(Y \mid X = x)$ 
...
$$\hat{f}_i = \frac{\sum_{j} y_j K\left(\frac{x_j - x_i}{h}\right)}{\sum_{l} K\left(\frac{x_l - x_i}{h}\right)}.$$
...
$$S_{ij} = \frac{K\left(\frac{x_j - x_i}{h}\right)}{\sum_{l} K\left(\frac{x_l - x_i}{h}\right)}.$$
The rows sum to 1 by construction, and the diagonal elements are 
$$S_{ii} = \frac{K(0)}{\sum_{l} K\left(\frac{x_l - x_i}{h}\right)}.$$
...
parameter $\lambda$. This representation allows for efficient computation 
with splines, and it will become clear why it is not necessary to compute 
$\mathbf{S}$ or even its diagonal elements, but the trace is nevertheless 
