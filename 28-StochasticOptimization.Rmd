...
suitable even if data is not Gaussian. It can be a suitable loss
...
model, that is, whether we want a good global fit as for the log-likelihood
loss, a fit of the mean value as for the squared error loss, or a fit of the
median or another quantile as for the check loss.
...
The precise way that the learning rate decays is known as the *decay
...
lacking the factor $\mu(x_i, \beta)$. With $Z$ uniformly distributed on
$(-1, 1$), the distribution of $\mu(Z, (2, 3))$ has range between
...
factor---than the log-likelihood gradient due to the mutiplication by $\mu(x_i,
...
specific challenge to find a good schedule---and even just to choose
the three parameters when we use the power law schedule.
...
The stochastic gradient algorithm is one such algorithm that "cycles through the
data" and uses only a random fraction of data points for each computation. In
its basic version presented first, it uses only a single data point in each
...
nonparametric bootstrapping. Just as in the online setting, the sequence
of learning rates, $\gamma_n$, is a tuning parameter of the algorithm.
...
news article is shared on social media. The data will be subjected to
...
using a toy problem makes it easier to understand in detail how the
different algorithms behave.
...
was not reached completely after the 50 epochs that took some time to
compute.
...
subsetting `X[i, ]` to the $i$-th row is actually responsible for a large
...
the R implementation that slicing out a row from a bigger matrix cannot
be done without creating a copy of that row, and this is why this
...
pure R implementation somewhat. Another way to circumvent some of the
...
mini-batches is ubiquitous, and momentum will be introduced to
illustrate a variation with memory. Finally, the Adam algorithm uses
...
The implementation uses `batch()` as the default update function, and we
...
assumes that the index argument $i$ is a single number and not a vector.
It computes for a vector $i$ a matrix containing the different gradients
as columns. We therefore reimplement `ls_model()` so that `grad()`
computes the average of the gradients as it is supposed to for a
mini-batch.
...
rate. The practical implication is that a considerable amount of tuning
is necessary, when the algorithms are applied.
...
for $j = 1, \ldots, p$. Thus if we sample a random index, $\iota$, from $\{1,
\ldots, N\}$ it holds that $\\E(x_{\iota j} (y_{\iota} - \hat{\beta}{}^Tx_{\iota})) = 0$, 
where the expectation is w.r.t. $\iota$. With
...
$(y_{\iota} - \hat{\beta}{}^Tx_{\iota})^2$, must be roughly independent
of $x_{\iota}$, which is not guaranteed by the least squares fit alone,
...
function from R, data would inevitably be copied. And since slicing of the
matrix in C++ does not create a copy of data, the implementation of `grad_cpp()`
avoids the data copying problem that matrix slicing in R incurred on the
...
achieved simply by adding one outer loop to `grad_cpp()` above, which
loops over mini-batches and updates the parameter vector.
...
(ref:Rcpp-fig) Convergence of squared error loss on the news article dataset for four algorithms: R implementation of mini-batch stochastic gradient with a batch size of 1000 and a fixed learning rate (`mini`), the same mini-batch algorithm but with the gradient implemented in Rcpp (`Rcpp_grad_mini`) and with the entire epoch implemented in Rcpp (`Rcpp_epoch_mini`), and the basic stochastic gradient algorithm with the entire epoch implemented in Rcpp (`Rcpp_epoch`).
...
gradient as a C++ function makes the convergence faster compared to the
pure R implementation, but turning the entire epoch into a C++ function
results in little if any further improvement. However, the figure also
shows that the basic stochastic gradient algorithm (mini-batch size
equal to one) converges at a rate comparable to that of the mini-batch
algorithm with mini-batch size 1000 when the entire epoch is in C++.
Thus any benefit of the mini-batch algorithm with mini-batch size 1000
seems to be largely due to the inefficiency of the basic algorithm in
pure R. Experiments using the `batch_cpp()` epoch implementation (not
shown) suggest that the mini-batch algorithm does have a real edge over
the basic algorithm, but with much smaller mini-batch sizes, e.g.
$m = 20$, cf. Exercise \@ref(exr:mini-batch-size).
...
a simple extension of `batch_cpp()` with one additional outer loop over
...
but using `dqsample_int() / dqsample.int()` to sample the same
permutations in both cases.
...
The two most notable overall differences between `stoch_grad()` and `sg_cpp()`
is that `sg_cpp()` is low level and very problem specific, while `stoch_grad()`
is high level and generic. It is surely possible to refactor the C++
code to write a more generic stochastic gradient algorithm, that would
rely on other functions for computing e.g. the gradient. However, to
retain performance those functions would need to be implemented in C++
as well.
...
The pure R implementation (`sg`) is, unsurprisingly, the slowest, but only about 
a factor 3 slower than the full Rcpp implementation (`sg_cpp`). The
Armadillo based implementation is somewhat slower, though still faster
than the pure R implementation.
...
This reveals some bigger differences. With a fixed number of epoch
iterations, the total amount of computations does not change
substantially when changing the mini-batch size from 50 to 1, but the
runtimes of `stoch_grad()` and `grad_cpp()` increase notably. The C++
implementations `batch_cpp()` and `sg_cpp()` mostly retain the run
time they had for $m = 50$. The runtime of the Armadillo implementation
is not affected that much either, but it is still about a factor 2 
slower than the two other C++ based implementations.

In summary, when the mini-batches are not too small, the improvements on
runtime by using Rcpp were relatively small---about a factor 2 to 3.
However, when the mini-batch size is small or even one, the runtime of 
the pure R implementation is a factor 20--30 slower than the full Rcpp implementation
or the Rcpp implementation of the entire epoch update. Using
`batch_cpp()` for the epoch updates strikes a good compromise in this
particular example, by being runtime competitive but with the main
control structures of the algorithm in R.
