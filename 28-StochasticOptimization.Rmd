...
particular, to avoid getting stuck in a local minimum. Stochastic gradient 
descent is another example, which compute descent directions from  
approximate gradients using random subsets of the data. 
...
```{r}
alpha_seq <- seq(0, 2, 0.01)
beta_seq <- seq(8, 12, 0.01)
...
SGD <- function(par, 
  ...
  repeat {
    ...
    if((value - value0) <= epsilon * (value + epsilon)) {
...
SGD(c(2, 9.2), H, grad_h, n_iter_no_change = 10, n = n, cb = SGD_tracer$trace)
```
