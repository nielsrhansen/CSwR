...
memory usage, the limitations of finite precision arithmetic, and the 
value of approximate solutions that may be theoretically suboptimal but 
sufficiently accurate for any pratical purpose.
...
developed and analyzed in cycles that resemble how software 
...
to [Advanced R](https://adv-r.hadley.nz/) by Hadley Wickham. In fact, direct 
references to that book are given for detailed explanations of R programming
language concepts such as functional programming and object oriented programming.
...
of already existing implementations.
...
used by `hist` chooses, for instance, breaks in $-3$ and $3$, which results in 
the two extreme 
...
library(MASS) # Includes the function `kde2d`
```
...
In statistics it is quite interesting in itself that we can make the computer 
...
(ref:vMdenscap) Density for the von Mises distribution with parameters $\kappa = 1$ and $\nu = 0$ (black), $\kappa = 2$ and $\nu = 1$ (red), and $\kappa = 0.5$ and $\nu = - 1.5$ (blue).
...
(ref:vMdatacap) Histogram of `r length(x)` simulated data points from a von Mises distribution with parameters $\kappa = 0.5$ and $\nu = - 1.5$. A smoothed density estimate (red) and the true density (blue) are added to the plot.
...
Bayesian computations) depend on a data set of size $n$, and for large
scale data sets, algorithms that scale like $O(n)$ are really the only 
...
This section illustrates how to fit a mixture of von Mises distributions to the 
...
```{r vMfit, dependson=c("phipsiLoad")}
...
says (see `?movMF`). In this case it means that `vM_fit` is a list with a class 
...
itself, and for `movMF()` such information is found in a list entry of `vM_fit`.
...
number of 100). The relative tolerance used for the stopping was $1.49 \times 10^{-8}$.
The `converge` entry of the list is `TRUE`, which for this algorithm simply 
indicates that the stopping criterion was used (`converge` is  a control argument,
and if it is `FALSE` the algorithm is run for a fixed number of iterations).

The relative tolerance criterion for stopping in iteration $n$ is 
...
optimization proceeds. Below a different method for setting the 
starting value is chosen, which contains a random component. Here we consider 
the results in four different runs of the entire algorithm with different random 
initialization. We also decrease the number of maximal
...
the EM-algorithm that every step of the algorithm will increase the log-likelihood.
...
real data analysis -- most importantly my making it possible to compute maximum
...
can be computed in $pn$-operations this may still be too much for an optimization
algorithm that does these computations in each iteration. Such large scale 
...
parts of data in each iteration, and which are now the only way to fit sufficiently 
