...
When implementing algorithms we must consider the trade-off between precision 
...
to need calibration to yield a good trade-off between smoothness
...
to evaluate the trade-off between time and memory complexity on one
...
area when it comes to developing computationally tractable and novel ways of fitting
smooth densities or conditional densities to multivariate
...
# rmovMF represents observations as elements on the unit circle
...
The `rmovMF` function actually implements simulation from a mixture distribution 
...
a simple average. Note that we will use a large number of observations, 50,000 in
...
\[
\int_a^b f(x) \mathrm{d} x \approx \sum_i f(x_i)w_i
\]
...
#| fig-cap: "Histogram of protein backbone angles with a smoothed density
#|   estimate (red) and the fitted mixture of von Mises distributions (blue)."
hist(angle$psi, breaks = seq(-pi, pi, length.out = 15), prob = TRUE)
rug(angle$psi)
...
Numerical optimization can fail for a number of reasons. For one, the algorithm
