...
```{Rcpp, Rcpp_chunk, ref.label=knitr::all_rcpp_labels(), include=FALSE}
```

```{r Rcpp_depends, include=FALSE}
## The dependson functionality for cached Rcpp-chunks appears not to work.
## Every run triggers the evalution of all chunks dependent on Rcpp-chunks.
## I belive this is caused by the way the knitr function 'block_exec' 
## treats non-R engines, and that there is the same problem for all 
## non-R engines. For non-R engines, 'block_cache' is always called if the chunk 
## is cached, and this results in deletion (in purge_cache) of the cache
## for this chuck and all dependent chunks. This might be fixed by 
## checking if the cache exists for non-R engines before calling 
## 'block_cache'.

## This chunck does nothing but ensure that if changed, then all chunks
## dependent upon this chunk are evaluated. This is a hack!
1
```
...
practical challenges reveal themselves, such as actual computation time and 
...
simulation of random variables and optimization play a role in statistics. If
you have no intention of moving beyond the role of a data analyst that rely 
...
as well, hence the need for automatic, robust and well calibrated smoothing 
procedures that call for a minimum of human effort. 
...
curve(exp(2 * cos(x - 1)) / phi(2), -pi, pi, col = "red", lwd = 2, add = TRUE)
curve(exp(0.5 * cos(x + 1.5)) / phi(0.5), -pi, pi, col = "blue", lwd = 2, add = TRUE)
```
...
terms of its frequentistic performance under various data sampling models. As 
is discussed in the next section, simulation is a tool of fundamental importance for 
...
Relations to Complex sampling models. Ex Bayes, MCMC, Stan.

## Optimization

Likelihood optimization for mixtures of the von Mises distribution. Direct 
implementation using `optim` and the density implementations. Alternatively
using the EM-algorithm as implemented in movMF.
...
vM_fit <- movMF(psi_circle, 2, control = list(verbose = TRUE, maxiter = 10, start = "S"))
```
...
density(phipsi$psi, bw = "SJ", cut = 0) %>% 
  lines(col = "red", lwd = 2)
curve(dvM(x, vM_fit$theta, vM_fit$alpha[1]), -pi, pi, add = TRUE, col = "blue", lwd = 2)
```

### Large scale optimization

Relations to complex optimization problems. Ex. deep learning, tensorflow.
...
Use your function to construct *breakpoints* for the histogram for different values of `h`, and compare with the histograms obtained in Exercise \@ref(plotHist).  
```

```{exercise}
If there are no ties in the data set, the function above will produce breakpoints with `h` 
observations in the interval between two consecutive breakpoints (except the last two perhaps). If there are ties, the function will by construction return unique breakpoints, but there may be 
...
Modify the `myBreaks` function with this intention and so that is has the following properties:
