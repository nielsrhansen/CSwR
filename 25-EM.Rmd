...
that it is one of the core optimization algorithms used for computing maximum-
...
som theoretical results as well as practical implementations for computing 
...
The term EM algorithm is usually reserved for the specific algorithm that maximizes
 $Q(\cdots \mid \theta_n)$ in the M-step, but as we have seen, there is 
absolutely no reason to insist on the M-step being a maximization. A choice 
of descent direction of $Q(\cdot \mid \theta_n)$ and a step-length 
guaranteeing sufficient descent will be enough to give a descent 
...
phat <- EM(c(0.3, 0.3), c(85, 196, 341), epsilon = 1e-20, trace = EM_tracer$trace)
```
...
$$\hat{\mathcal{I}}(\theta_0) =  \sum_{i=1}^n \big(\nabla_{\theta} Q_i(\theta_0 \mid \theta_0) - n^{-1} \nabla_{\theta} \ell(\theta_0)\big)^2$$

has mean $\mathcal{I}(\theta_0)$ and is thus almost an unbiased estimator of the
Fisher information. It does have the right mean, but it is not an estimator 
as $\theta_0$ is not known. Though if we plug in an estimator, 
...
Fisher information in principle simplifies to 
...
individual speciments and tabulated according to phenotype, we implement
...
Though the EM map $\Phi$ might not have a simple analytical expression, 
